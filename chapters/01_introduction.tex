...
Before addressing these questions, we could just say that mechanising mathematics is an enjoyable task.
...
\begin{enumerate}
\item result in a machine-checked work,
...
Furthermore, formalising a system using a proof assistant enhances reusability of such formalisation.
...
The \textit{Autosubst} library \cite{AutosubstSchafer} used in our work is a great example of these advancements in the research of mechanised metatheory.
...
\begin{enumerate}
\item system $\LamM$, namely its reduction theory, its typing system and standard results like subject reduction or confluence;
\item the canonical subsystem of $\LamM$;
\item the conservativeness of $\LamM$ over its canonical subsystem;
\item the isomorphism between the canonical subsystem of $\LamM$ and the simply typed $\Lam$-calculus.
\end{enumerate}
...
Our main contribution with this dissertation is a mechanisation in the \textit{Rocq Prover} of the system $\LamM$.
This includes a result of subject reduction, a theorem of conservativeness, an isomorphism of its subsystem with the simply typed $\Lam$-calculus and confluence results derived from $\Lam$-calculus.
We know of no other works formalising this metatheory.
The formalised body of work provides a computer-verified and highly accessible foundation for future developments, using version 9.0.0 of the \textit{Rocq Prover} and version 1.9 of the \textit{Autosubst} library, and can be found in an open-source \textit{GitHub} repository:
\[ \text{\href{https://github.com/thetruezau/LambdaM}{https://github.com/thetruezau/LambdaM}.} \]
...
\[\begin{tikzcd}
	&&& {\boxed{\LamM}} \\
	{\boxed{\Lam}} && {\boxed{\LamV}} && {\boxed{\LamCan}} \\
...
\paragraph{\Cref{c:multiary}} introduces the system $\LamM$ and its canonical subsystem $\LamCan$.
...
Using this system, we will establish a theorem of conservativeness.
