...
It is interesting to notice that the resultant nameless $\Lam$-term is the same up to a renaming of free variables.
...
We reorder the context $\Delta$ according to a rename $\xi$ by declaring that \lst$Gamma = (xi >>> Delta)$, where \lst$>>>$ denotes forward composition (this is a composition operation defined by \textit{Autosubst}).
