...
\end{corollary}
\begin{proof}
  Recall \cref{h_is_multistep}. Given $\LamM$-terms $t, t_1, t_2$, we have the following:
  \[
    ...
    \begin{tikzcd}
      ...
      i(p(t_1))=h(t_1) \arrow[rd, two heads, dashed, "\beta h"] && i(p(t_2))=h(t_2) \arrow[ld, two heads, dashed, "\beta h"] \\
      & \exists \ i(t') &
    \end{tikzcd}
  \]
  
  In the latter diagram, the existence of such $t'$ comes from the fact that $\LamV$ is confluent and from using \cref{conservativeness} to lift the following diagram to $\LamM$.
  \[
