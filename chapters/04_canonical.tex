...
The notation used for this system intentionally points towards its vectorial style.
The naming of this system is motivated by two reasons.
On the one hand, as we will see in this chapter, it is isomorphic to the canonical subsystem $\LamCan$ seen in the previous chapter.
On the other hand, as we will see in the next chapter, it is also isomorphic to the simply typed $\Lam$-calculus.

System $\LamV$ is an independent representation of the canonical subsystem of $\LamM$ (in opposition to $\LamCan$).
We will give a complete proof for the isomorphism between both systems in the second section of this chapter.
In the third section, we prove a conservativeness result that links reduction in system $\LamM$ and system $\LamV$.
In turn, such result proves that $\LamM$ is a conservative extension of $\LamCan$.
...
\begin{remark}
  ...
  In both constructors, every occurrence of the variable $x$ is bound (and not free).
  ...
  Even more, $\LamV$-terms have two constructors that introduce occurrences of free variables: $var(x)$ and $app_{v} (x, u, l)$.
\end{remark}
...
\begin{definition}
  ...
  \end{align*}
  where the list append, $l + l'$, has the expected behaviour (as in $\LamM$).
\end{definition}

Now follows a ``strange'' definition for a substitution operation, because, in the critical substitution case $app_v(x, u', l)[x := u]$, the usual behaviour for the substitution would result in an incorrect $\LamV$-term $app_v(u, u', l)$.
...
\begin{remark}
  ...
  % proof search
  System $\LamV$ offers thus an advantage in comparison to the $\Lam$-calculus, where a description of $\beta$-normal forms is more elaborated.
\end{remark}
...
We should briefly highlight the sequent calculus features found in the previous typing rules.
Firstly, it is clear that we have rules not only for the introduction of implication on the right (Abs), but also for the introduction of implication on the left (Cons).
Secondly, we may also see rule App$_v$ as introducing an implication $A \supset B$ on the left and immediately contracting it with $x: A \supset B$.
% of implication ($A \supset B$) on the left followed by a contraction.
Thirdly, in the rule App$_\lambda$, we have two simultaneous cuts happening (the first and second premises cut on $A$, and, the first and third premises cut on $B$).
Indeed, we have a rather specific typing system, but still with a strong sequent calculus flavour.

% ---
% 123
% ---

\section{$\LamV$ vs $\LamCan$}
...
The following diagram summarises the connection between the defined maps and map $h$ defined in \cref{c:multiary}.
...
\[
  \begin{tikzcd}
    ...
    \pmb{\vec \lambda} \arrow[r, "i"'] & \LamCan                                      
  \end{tikzcd}
\]

% We show some useful lemmas for the following results.
We begin by proving that the above diagram is commutative.
...
Canonical terms are not closed for the substitution operation in $\LamM$.
...
The following technical lemma says that we can derive the compatibility rules of the system $\LamV$ given the canonical closure of a compatible relation on $\LamM$.
...
\begin{proof}
  ...
  Then, from \cref{canonical_closure}, we have that there exist $\LamM$-terms $t_1$ and $t_2$ such that $h(t_1) = i(t)$ and $h(t_2) = i(t')$ and $t_1 \to_R t_2$.
...
\end{theorem}
\begin{proof}
  The proof proceeds by induction on the relation $\to_{\vec \beta}$ on $\LamV$-expressions.
...
\begin{lemma}[Append typing rule]
...
Our argument for the isomorphism between the canonical subsystem $\LamCan$ and $\LamV$ ends here.
From now on, we will use the self contained representation, system $\LamV$, to prove results about the canonical subsystem in $\LamM$.
...
\begin{proof}
  ...
  For this implication, we first show how a reduction $t \to_{\beta h} t'$ in $\LamM$ is directly translated into a reduction $p(t) \twoheadrightarrow_{\vec \beta} p(t')$ in $\LamV$. 
  ...
  \[
    i(u) \twoheadrightarrow_{\beta h} i(u') \implies
    \underbrace{p(i(u))}_u \twoheadrightarrow_{\vec \beta} \underbrace{p(i(u'))}_{u'}.
  \]
\end{proof}

% Although a relation $(\twoheadrightarrow_{\beta})_c$ has not been considered before, it can be thought of as a multistep reduction relation for $\LamCan$.

\begin{corollary}[Conservative extension]
  $\LamM$ is a conservative extension of $\LamCan$.
...
\end{corollary}
\begin{proof}
  Immediate by \cref{canonical_reduction_iso,conservativeness}.
\end{proof}

As another immediate application of the conservativeness result just proved, we can derive subject reduction for $\LamV$ from $\LamM$ (already proved as \cref{type_preservation}).

\begin{corollary}[Subject reduction for $\LamV$]
  \label{can_subject_reduction}
  Given $\LamV$-terms $t$ and $t'$, the following holds:
  \[
...
\begin{proof}
  ...
  \[
    ...
    \begin{tikzcd}
      ...
      & t \arrow[ld, two heads, "\beta h"] \arrow[rd, two heads, "\beta h"] & \\
      % t_1 \arrow[d, two heads, "h"] & i(p(t))=h(t) \arrow[ld, two heads, "\beta h"] \arrow[rd, two heads, "\beta h"] & t_2 \arrow[d, two heads, "h"] \\
      t_1 \arrow[d, two heads, "h"] & i(p(t)) \arrow[ld, two heads, "\beta h"] \arrow[rd, two heads, "\beta h"] & t_2 \arrow[d, two heads, "h"] \\
      h(t_1)=i(p(t_1)) \arrow[rd, two heads, dashed, "\beta h"] && i(p(t_2))=h(t_2) \arrow[ld, two heads, dashed, "\beta h"] \\
      & \exists \ i(t') &
    \end{tikzcd}
  \]
  
  In the latter diagram, the existence of such $t'$ comes from the fact that $\LamV$ is confluent.
  This can be seen by applying first \cref{conservativeness} to our hypothesis $t \twoheadrightarrow_{\beta h} t_1$ and $t \twoheadrightarrow_{\beta h} t_2$ getting that $p(t) \twoheadrightarrow_{\vec \beta} p(t_1)$ and $p(t) \twoheadrightarrow_{\vec \beta} p(t_2)$.
  \[
    ...
    \begin{tikzcd}
      & p(t) \arrow[ld, two heads, "\vec \beta"] \arrow[rd, two heads, "\vec \beta"] & \\
      p(t_1) \arrow[rd, two heads, dashed, "\vec \beta"] && p(t_2) \arrow[ld, two heads, dashed, "\vec \beta"] \\
      & \exists \ t' &                             
    \end{tikzcd}
  \]
  Second, we use the converse implication of \cref{conservativeness} to lift the previous diagram, mapping each term to its image by $i$ and each $\twoheadrightarrow_{\vec \beta}$ step to a $\twoheadrightarrow_{\beta h}$ step.
  Finally, we recall \cref{h_is_multistep} to justify the vertical arrows in the first diagram and \cref{comm_i_p_h} to justify each equality.
\end{proof}
...
In the next subsection we describe in more detail the approach used to define the substitution operation for this system.
...
Despite being a bureaucratic result, it helps simplifying further proofs (such as \cref{i_step_pres} shown below) and reveals some benefits of formalising the general predicate of compatibility \lst$is_compatible$.
...
More specifically, the first line of the proof is adding to the context the fact of \lst$step_can$ being a compatible relation for $\LamV$-terms.

\subsection{\lst$Conservativeness.v$}

This module is only about the proof for the conservativeness theorem.
The mechanised theorem follows exactly the proof given diagrammatically in~\cref{conservativeness}.
We explicitly divided the proof into two parts, \lst$conservativeness1$ and \lst$conservativeness2$, for each of the two concerned implications.
...
\subsubsection{Mechanising a nonstandard substitution operation}

One of the most peculiar definitions in system $\LamV$ is the substitution operation (\cref{canonical_substitution}).
As said before, we have an unusual behaviour for the constructor $app_v$.
...
However, proving such lemmas was crucial to enjoy the simplification tactics and automation provided from the library for the mechanised inductive type of the $\LamV$-terms.
