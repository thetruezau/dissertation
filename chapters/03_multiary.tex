...
\begin{lemma}[Substitution typing rules]
...
\section{The canonical subsystem $\LamCan$}

The canonical subsystem is a system within $\LamM$ containing only terms in $h$-normal form.
In this section we see how to equip such terms with an appropriate notion of $\beta$-reduction and appropriate typing rules derived from the notions of reduction and typing of $\LamM$.
This is a nontrivial task because, for instance, the canonical terms are not closed under the substitution operation defined in $\LamM$.
...
Again, one may find a mechanised proof for this claim in the script repository.
...
\begin{corollary}[Confluence of $\twoheadrightarrow_h$]
  For every $\LamM$-terms $t, t_1, t_2$ such that $t \twoheadrightarrow_h t_1$ and $t \twoheadrightarrow_h t_2$, there exists a $\LamM$-term $t'$ such that $t_1 \twoheadrightarrow_h t'$ and $t_2 \twoheadrightarrow_h t'$.
\end{corollary}
\begin{proof}
  Immediate by \cref{h_collapse,h_is_multistep}.
  The proof can be illustrated as follows.
  % https://q.uiver.app/#q=
\[\begin{tikzcd}
	& t \\
	{t_1} && {t_2} \\
	{h(t_1)} & {h(t)} & {h(t_2)}
	\arrow["{_h}"{pos=0.8}, two heads, from=1-2, to=2-1]
	\arrow["{_h}"'{pos=0.8}, two heads, from=1-2, to=2-3]
	\arrow["{_h}"'{pos=0.8}, two heads, from=1-2, to=3-2]
	\arrow["{_h}", two heads, from=2-1, to=3-1]
	\arrow["{_h}"', two heads, from=2-3, to=3-3]
...
Given $t, u \in Can$, let us see how to reduce $(\lambda x . t)(u, [])$.
...
We now conclude the presentation of the canonical subsystem $\LamCan$, by equipping canonical expressions with a typing relation, in the same spirit of~\cref{canonical_closure}.
...
We define a subset of $\LamM$-terms by restricting our syntax of expressions to $h$-normal ones.
Then, from the notions of reduction and typing on $\LamM$, derived notions for the restricted syntax were defined by using a map $h$ that collapses $\LamM$-terms into canonical terms.
...
Here, \lst$comp$ and \lst$comp'$ are the polymorphic relations that induce the compatibility closures on $\LamM$-terms and $\LamM$-lists, respectively.
...
Lemma weak_type_substitution Gamma t A :
  sequent (B.:Gamma) t A -> sequent Gamma u B -> sequent Gamma t.[u.:sigma] A).
\end{lstlisting}
...
This module contains the necessary definitions for the formalisation of the canonical subsystem $\LamCan$.
...
In the \textit{Rocq Prover}, we need to formally prove that canonical terms are closed for the $app$ operation and map $h$.
...
Then, we prove all the lemmas, propositions and theorems presented in the description of $\LamCan$.
...
The module ends with definitions for the reduction relation (recall~\cref{canonical_closure}) and typing rules (recall~\cref{canonical_typing}) for the canonical subsystem $\LamCan$.
\begin{lstlisting}[language=Coq]
...
The \lst$derive$ tactic would not generate the desired instances for the \lst$Rename$ and \lst$Subst$ classes, failing to iterate through the customised list type.
...
So far, our approach on the formalisation of the canonical subsystem of $\LamM$ was to view it as induced by map $h$, that is, defining reduction and typification using this map.
