...
First, we introduce some lemmata, in order to relate the mappings $\theta'$ and $\psi'$ with the @ operation and list append.
...
\begin{corollary}[Confluence of $\LamV$]
  ...
  The relation $\twoheadrightarrow_{\vec \beta}$ is confluent. 
\end{corollary}
\begin{proof}
...
\begin{corollary}[Confluence of $\LamM$]
  The relation $\twoheadrightarrow_{\beta h}$ is confluent.
\end{corollary}
\begin{proof}
...
\begin{theorem}[Soundness of $\theta$]
  \label{theta_admiss}
...
\begin{theorem}[Soundness of $\psi'$]
  \label{psi'_admiss}
...
\begin{corollary}[Subject Reduction for $\Lam$]
  Given $\Lam$-terms $M$ and $N$, the following holds:
  \[
...
To aid the reader, we describe a way to define such a function in the \textit{Rocq Prover}.
\begin{lstlisting}[language=Coq]
Fixpoint fold_left {A B: Type} (f: A->B->A) (l: list B) (a: A) : A :=
  match l with
  | [] => a
  | b::l => fold_left f l (f a b)
  end.
\end{lstlisting}
Fortunately, a user may easily import this definition from the \lst$Lists$ library.

Such representation of map $\theta$ using \lst$fold_left$ was an undesired consequence of the use of polymorphic lists in the definition for $\LamV$-terms.
...
Unfortunately, with this definition for $\theta'$, we had to repeatedly fold
\footnote{
  One should carefully read the context in which the word fold appears:
  when ``fold'' refers to a definition, it is related to the \textit{Rocq} tactic \lst$fold$;
  when ``fold'' refers to a list, it is related to the \lst$fold_left$ function.
}
its definition in our proof developments to make the goal more readable (hiding the calls to \lst$fold_left$).
