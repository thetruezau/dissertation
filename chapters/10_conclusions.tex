...
Our most important contribution is a mechanisation using the \textit{Rocq Prover} and the \textit{Autosubst} library of the following systems:
\begin{enumerate}
\item the multiary $\Lam$-calculus (system $\LamM$);
\item the canonical subsystem of $\LamM$ (system $\LamCan$);
...
\item conservativeness of $\LamM$ over its canonical subsystem;
...
Each system and relationship is explicitly replaced by a \textit{Rocq} script with the corresponding formalisation.

% ---------------
\begin{figure}[h]
  \centering
  \includegraphics[width=1\textwidth]{pictures/scripts_roadmap}
  \caption{Roadmap of scripts in the \textit{Rocq Prover}} \label{scripts_roadmap}
\end{figure}
% ----------
...
First, a PhD thesis \cite{AndrewAdams} that discusses many possibilities for mechanising a cut-free fragment of the sequent calculus and its relation with natural deduction, by experimenting with many proof assistants (including \textit{Coq} --- the former name of the \textit{Rocq Prover}) and binding techniques (including de~Bruijn indices).
...
Second, in the case of system $\LamV$, we would have nothing generated by \textit{Autosubst2}, as this system has an unexpected behaviour because of its nonconventional substitution operation.
...
Related with the choice of using the \textit{Autosubst} library, we recall the use of infinite contexts, or contexts as functions mapping natural numbers to types.
...
Our approach was to separate two different systems.
First, we defined the canonical subsystem of $\LamM$, given through subsets of expressions of $\LamM$ ($Can$ and $CanList$) closed under notions of reduction and typing.
Afterwards, we wanted a direct, yet isomorphic, representation of this canonical subsystem, with ``self-contained'' notions of substitution, reduction and typing.
This led us to the definition of the system $\LamV$, which, indeed, we proved to be isomorphic to the canonical subsystem of $\LamM$.
...
An alternative could be the subset types provided in \textit{Rocq}.
We did some experiments along this path, but this would rapidly become tiring because of the constant need to provide the proof object of a predicate defining the subset.
...
A distinct and possible new line of work would involve generalising our typing systems beyond simple types - for instance, by incorporating polymorphism or dependent types.
...
From our experience using the \textit{Autosubst} library, we could suggest improvements in the automated tactics for deriving the substitution operation and substitution lemmas (motivated by system $\LamV$ with its unconventional substitution).
An example of this could be a refinement of the library to deal with relaxed versions of the $\sigma$-calculus, that is, syntaxes that do not satisfy every rewriting rule of the $\sigma$-calculus.
% Compare step-by-step versions of cut reductions (like lambda bar) with our versions that create strange fragments?
% what is the canonical explicit fragment of lambda bar?
% We also ask ourserlves about other syntaxes with substitution operations that combine other operations inside.
It would also be interesting to have a shared repository containing many formalised systems and classical results using \textit{Autosubst}.
% that could easily be used in other developments.
This would allow other developments similar to ours to be used and improved inside a community.
In our case, we derived the confluence of $\LamM$ from the confluence of the $\Lam$-calculus.
Strong normalisation for $\LamM$ could potentially be explored in this way.
