...
\begin{frame}
	...
	\begin{itemize}
		\item<4-> Long and tedious proofs (often by structural induction)
		\item<5-> A good target for mechanisation
	\end{itemize}
	
	\onslide<6->{ Why the multiary $\Lam$-calculus? }
	\begin{itemize}
		\item<7-> Results from a simple adaptation of applicative terms $t u \to t(u, l)$
		\item<8-> Typing rules have interesting logical content (sequent calculus!)
	\end{itemize}
\end{frame}
...
\begin{frame}
  \begin{itemize}
  \item<1-> Why mechanise?
  \item<2-> Why mechanise metatheory?
  \item<3-> Why the multiary $\lambda$-calculus?    
  \end{itemize}
\end{frame}
...
\begin{frame}
	\frametitle{Christmas surprise!}
	\Large{ A Linear Algebra theorem mechanised! }
\end{frame}
