...
% \title[ALC Meeting 2025] % optional
\title[Days in Logic] % optional
{On the mechanisation of the multiary lambda calculus and subsystems}

\author[Miguel Alves]
{Miguel Alves}

\institute[UMinho]{Departamento de Matemática \\ Universidade do Minho}
...
\begin{frame}
	...
	Why mechanise?
	\begin{itemize}
		\item<1-> Computer verification of proofs
		\item<2-> Use of automation and libraries (ex: mathlib for lean)
	\end{itemize}
	
	\onslide<3->{ Why mechanise metatheory? }
	\begin{itemize}
		\item<4-> Proofs are often long and tedious (often by structural induction)
        % \item<5-> A good target for mechanisation
	\end{itemize}
	
	\onslide<5->{ Why the multiary $\Lam$-calculus? }
	\begin{itemize}
		\item<6-> Computational interest: applicative terms can now apply to lists of arguments $t(u, l)$
		\item<7-> Logical interest: typing rules exhibit a sequent calculus flavour, but also connect to natural deduction with generalised eliminations
		% but can also be precisely related to natural deduction with gen elims
	\end{itemize}
\end{frame}
...
\subsection{Multiary $\Lam$-calculus ($\LamM$)}
\begin{frame}
  ...
  \begin{block}{Typing rules}<1,8->
    ...
    \only<8->{ Typing rules are isomorphic to a fragment of the sequent calculus (similar to system $\overline\lambda$ and the focused sequent calculus $LJT$). }
  \end{block}
\end{frame}

\begin{frame}
  \frametitle{Typing system of $\LamM$}
  \begin{definition}[Typing rules for $\LamM$-terms]
    \[
      \begin{prooftree}
        \infer0[Ax]{ x:A, \Gamma \vdash x:A } 
      \end{prooftree}
      \qquad
      \begin{prooftree}
    ...
    \[
      \begin{prooftree}
        ...
        \infer3[mElim]{ \Gamma \vdash t(u, l) : C }
      \end{prooftree}
    \]
    \[
      \begin{prooftree}
  ...
  \only<2->{ The sequent deriving lists creates only derivations where the formula in focus is a chain of implications.
  	\[
  	\begin{prooftree}
  		\hypo{ \Gamma \vdash B_1 }
  		\hypo{ \Gamma \vdash B_2 }
  		\hypo{}
  		
  		\ellipsis{}{ \Gamma ; ... \vdash C }
  		\infer2[Left*]{ \Gamma ; B_2 \supset ... \vdash  C }
  		\infer2[Left*]{ \Gamma ; B_1 \supset B_2 \supset ... \vdash  C } 
  	\end{prooftree}
  	\]
  }  
\end{frame}
...
\begin{frame}
  \frametitle{ The generalised elimination spirit behind $\LamM$ }
    Notice the similarities between the multiary elimination rule and the generalised elimination rule from von Plato.
    
    \[
    \begin{prooftree}
    	\hypo{ \Gamma \vdash t : A \supset B }
    	\hypo{ \Gamma \vdash u : A }
    	\hypo{ \color{blue} \Gamma ; B \vdash l : C }
    	\infer3[mElim]{ \Gamma \vdash t(u, l) : C }
    \end{prooftree}
    \]
    \vspace{10pt}
    \[
      \begin{prooftree}
        \hypo{ \Gamma \vdash A \supset B }
        \hypo{ \Gamma \vdash A }
        \hypo{ \color{blue} \Gamma , B \vdash C }	
        \infer3[gElim]{ \Gamma \vdash C }
      \end{prooftree}
    \]
\end{frame}
...
\begin{frame}
  \frametitle{$h$-normal forms and $\LamCan$}
...
\begin{frame}
  \frametitle{The commuting conversion on derivations behind $h$}
  \[\scriptsize
    \begin{prooftree}
        \hypo{ \Gamma \vdash t : A \supset B }
        \hypo{ \Gamma \vdash u : A }
        \hypo{ \Gamma ; B \vdash l : C \supset D }	
        \infer3[mElim]{ \Gamma \vdash t(u, l) : C \supset D }
        \hypo{ \Gamma \vdash u' : C }
        \hypo{ \Gamma ; D \vdash l' : E }
        \infer3[mElim]{ \Gamma \vdash t(u, l)(u', l') : E }
      \end{prooftree}
    \]
    \[ \rightsquigarrow \]
    \[\scriptsize
      \begin{prooftree}
        ...
        \hypo{ \Gamma \vdash u' : C }
        \hypo{ \Gamma ; D \vdash l' : E }
        \infer2[Left*]{ \Gamma ; C \supset D \vdash u'::l' : E }
        \infer[dashed]2[Cut*]{ \Gamma ; B \vdash l+(u'::l') : E }
        \infer3[mElim]{ \Gamma \vdash t(u, l+(u'::l')) : E }
      \end{prooftree}
    \]
\end{frame}
...
\subsection{Canonical $\Lam$-calculus ($\LamV$)}
\begin{frame}
  ...
  \begin{definition}[Reduction rules]<1,5-> \vspace{-15pt}
    \begin{align*}
      ...
      \only<6-7>{ (\vec \beta_2) & \qquad app_\lambda (x . t, u, v::l) \to {\color{red} t[x := u](v, l)} }
...
\begin{frame}
  ...
  $\LamCan$ and $\LamV$ are \textbf{isomorphic} at the level of both typing and reduction.
...
\begin{frame}
  ...
  \begin{theorem}[Conservativeness]
    ...
    \[ t \twoheadrightarrow_{\vec \beta} t' \text{ in } \LamV \iff i(t) \twoheadrightarrow_{\beta h} i(t') \text{ in } \LamM. \]
  \end{theorem}

  \begin{corollary}[$\LamM$ is a conservative extension of $\LamCan$]<2->
    % In other words,
    For every $t, t' \in Can$, 
    \[ t \twoheadrightarrow_{\beta^{Can}} t' \text{ in } \LamCan \iff t \twoheadrightarrow_{\beta h} t' \text{ in } \LamM, \]
...
\begin{frame}
  ...
  $\LamV$ and $\Lam$ are \textbf{isomorphic} at the level of both typing and reduction.
...
\subsection{The mechanised work}
\begin{frame}
  \frametitle{The mechanised work}
  \begin{figure}[h]
  	\centering
  	\includegraphics[width=0.9\textwidth]{pictures/scripts_roadmap}
  \end{figure}
  
  \only<2->{
  \begin{figure}[h]
  	\centering
  	\includegraphics[width=0.7\textwidth]{pictures/screenshot_loc}
  \end{figure}}
\end{frame}
...
\begin{frame}
  ...
  \begin{itemize}
  \item<1-> A simple, yet, computationally interesting idea like changing the syntax of the $\Lam$-calculus application has a logical meaning involving sequent calculus and natural deduction with generalised eliminations.
  ...
  \item<3-> Mechanising metatheory can be boosted by using the available libraries (\textit{Autosubst} in our case) and generalisation of many concepts is a good path to automation of proofs and code reusability.
  \end{itemize}
\end{frame}
