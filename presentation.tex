...
\begin{frame}
  ...
  \begin{definition}[Typing rules]<1,8->
...
\begin{frame}
  ...
  is to bring either a variable or an abstraction to the head of applicative terms.

  \onslide<2->{ We will identify the subsets $Can$ (of terms) and $CanList$ (of lists) in $h$-normal form, having applicative terms restricted to \[ x(u, l) \quad \text{ or } \quad (\lambda x . t)(u, l). \] }
...
\begin{frame}
  ...
  \begin{itemize}
  \item<1-> A simple, yet, computationally interesting idea like changing the syntax of the $\Lam$-calculus aplication has a logical meaning involving sequent calculus.
  \item<2-> A mechanisation may lead to a different representation of the object theory.
    % \item<3-> Mechanisation works can help clarifying
  \item<3-> Mechanising metatheory can be boosted by available libraries and modularisation may help in reusing available work.
  \end{itemize}
\end{frame}
